...
function useAdminCreateProduct() {
  ...
  return useMutation({
    mutationFn: async (product) => {
      ...
      if (!actor) throw new Error("Actor not ready");
      if (!adminToken)
        throw new Error("No admin session — please log in again.");
...
function useAdminAddBanner() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const result = await actor.adminAddBanner(token, name, imageUrl);
      if (result.__kind__ === "err") throw new Error(result.err);
...
function useAdminDeleteBanner() {
  ...
  return useMutation({
    mutationFn: async (id) => {
      ...
      if (!token) throw new Error("No admin session — please log in again.");
      const numId = parseBigInt(id);
      const result = await actor.adminDeleteBanner(token, numId);
      if (result.__kind__ === "err") throw new Error(result.err);
      return result.ok;
    },
    onSuccess: () => {
